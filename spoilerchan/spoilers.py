    ...
    async def spoilerchannel(
        ...
        *,
        ...
        if isinstance(channel, str):
            ...
            channel = utils.find(
                lambda chan: chan.name == channel_name, guild.channels
            ) or await spoilercat.create_text_channel(channel, overwrites=overwrites)
...
async def get_signup_chan(guild: discord.Guild) -> TextChannel:
    ...
    return utils.get(
        spoilercat.channels, name="spoiler-channels"
    ) or await spoilercat.create_text_channel(
        "spoiler-channels", topic="Get your spoilers here!", overwrites=overwrites
    )
...
async def get_signup_msg(channel: TextChannel) -> discord.Message:
    ...
    msg = await signups.history().find(
        lambda m: mention in m.content
    ) or await signups.send(content)
